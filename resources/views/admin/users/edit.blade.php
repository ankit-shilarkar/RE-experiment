...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            {{-- Projects --}}
            <div class="form-group">
                <label class="required" for="projects">{{ trans('cruds.user.fields.projects') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('projects') ? 'is-invalid' : '' }}" name="projects[]" id="projects" multiple required>
                    @foreach($projects as $id => $project)
                        <option value="{{ $id }}" {{ (in_array($id, old('projects', [])) || $user->projects->contains($id)) ? 'selected' : '' }}>{{ $project }}</option>
                    @endforeach
                </select>
